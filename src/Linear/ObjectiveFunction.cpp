...
ObjectiveFunction::ObjectiveFunction(const arma::mat& MPmat) :
    MP(MPmat), xPrev(arma::vec())
{
  // Initialize variable F and J so that they can be re-used in Evaluate(x)
  F.set_size(MP.n_rows);
...
ObjectiveFunction::ObjectiveFunction(const arma::mat& MPmat, const float alpha, const arma::vec& xPrev) :
    MP(MPmat), alpha(alpha), xPrev(xPrev)
{
  // Initialize variable F and J so that they can be re-used in Evaluate(x)
  F.set_size(MP.n_rows + xPrev.n_elem);

  // Jacobian of the function F = MP * x + alpha * I * (x - xPrev) is constant
  J = arma::join_cols( MP, alpha * arma::eye(xPrev.n_elem, xPrev.n_elem) );
...
{
  if (useTemporal)
    F = arma::join_cols( MP * x, alpha * (x - xPrev) );
  else
    F = MP * x;
